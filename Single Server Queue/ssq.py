...
Original file is located at
    https://colab.research.google.com/drive/1qGTeYxRvv-6pk4gkZ6H9PT3BWtIJHeDC
"""
...
class SSQ:
  def __init__(self,option):   #Initialization
    ...
    np.random.seed(0)   
    self.interarrivals= list(np.random.exponential(0.5,60))
  ...
  def departure1(self): ##depart from server-1
    ...
    if self.num_in_queue==0:  #if no customer in the queue
      ...
      if option==2:
        ...
        delay= self.clock- arrival
        self.total_delay+=delay
        self.next_departure1= self.clock+ self.service_times_in_queue.pop(index_min)  
      elif option==3:
...
option=int(input()) #1---FIFO, 2---SJF,  3----LIFO
s = SSQ(option)
s.start()